...
**dicha** 是一个像素风、2.5D 等距视角、**承认生活有重量**的个人物品管理 OS。
...
| **承诺** | dicha 永远不会变质——所有代码全开源，无 Enterprise Tier |
...
Apple Health / Journal 风格。打开 dicha 看到的是你的物品和记忆，不是游戏。
...
**MVP 包含 3 个房间（α 战略）：**
1. **衣橱**（深度）—— dicha 的"装饰灵魂"
2. **书房**（深度）—— dicha 的"内容灵魂" + 市集冷启动黄金
3. **杂物间**（浅）—— M1 第一个落地
...
**Week 11-12 自用**
- 自己每天用 dicha 录物品 ≥ 30 件，找痛点
- 调 AI 识别准确率 + 调诗的 tone
- 重构 + bug fix

**M1 验收：** dicha 在自己电脑上活起来，每天用它管理杂物。
...
4. **M3 别推迟发布** —— 截止日就是动力
5. **每周 1 天用 dicha（不开发只用）** —— 自己不用做出来一定是错的
...
这些是 dicha 的"基础设施作品"，没有它们不能 ship：
...
dicha 是一个 idea 的胜利——它的每一笔设计都来自具体的判断：
