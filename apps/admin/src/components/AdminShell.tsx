          ...
          <aside className="relative flex h-full w-[min(82vw,280px)] flex-col bg-sidebar-bg px-3 py-4 text-sidebar-ink shadow-float">
            <div className="mb-2 flex justify-end">
              <HeroButton
                ...
                aria-label="关闭导航"
                onClick={() => setMobileNavOpen(false)}
                className="grid size-9 place-items-center rounded-md bg-sidebar-ink/10 text-sidebar-ink transition-colors hover:bg-sidebar-ink/15"
              >
  ...
  return (
    <>
      <div className="flex items-center gap-3 px-2 py-2">
        <span className="grid size-9 place-items-center rounded-md bg-sidebar-ink/10 text-sidebar-ink">
      ...
      <div className="mt-4 shrink-0 rounded-md border border-sidebar-ink/10 bg-sidebar-ink/[0.04] p-3">
        <p className="truncate text-xs font-medium">{user.name}</p>
        <p className="mt-1 truncate text-[11px] text-sidebar-ink-soft">{user.email}</p>
        <HeroButton
          type="button"
          onClick={onLogout}
          className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md bg-sidebar-ink/10 px-3 py-2 text-xs text-sidebar-ink transition-colors hover:bg-sidebar-ink/15"
        >
...
function SidebarNavItem({
  ...
  return (
    <Link
      ...
      activeProps={{
        className:
          'bg-[color-mix(in oklab, var(--accent-foreground) 10%, transparent)] text-sidebar-ink',
      }}
