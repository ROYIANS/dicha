...
- Admin-enabled provider-directory models are platform-maintained catalog rows for user-facing BYOK providers. When an admin enables a provider, web should show that provider; when admin enables directory models for it, web should show those models. User-side model enable/disable is a personal routing preference and must not be treated as the admin visibility flag.
- User-facing `/settings/ai-providers` must not expose upstream model sync for admin-maintained built-in providers. Upstream sync remains a user-side action only for user-created custom providers.
- Users may add custom models under a visible provider and may delete only user-owned models: `custom` models or models under a custom provider. Admin-maintained directory models must not be deletable from the frontend and must be removed from user catalogs when the admin disables/removes them from the visible seed.
- Admin owns provider-directory model metadata, including avatar, display name, context window, model type, capabilities, extension parameters, and default parameter config. The user-facing settings surface may display that metadata but must not expose model metadata/configuration entry points for admin-maintained directory models. Users may edit this full metadata form only for custom models or models under custom providers.
- Directory model `parameterConfig` is the admin-maintained default parameter object. Admin and user-facing settings must expose it through form controls for common request parameters plus LobeHub-style extension-parameter controls, not as the primary raw JSON interface. User model config may store `parameterConfig` as a per-user override; invoke adapters receive `{ ...defaultParameterConfig, ...parameterConfig }` for user-provider models. Dicha official channel parameter config remains system-channel-owned and is not user-editable from frontend settings.
...
- Settings UI should allow sync/check for user-created custom `openai_models_endpoint` providers even before credentials are saved. Built-in provider-directory templates are synced in admin, not in the user frontend. Model discovery mirrors LobeHub's public catalog behavior: first try the provider `/models` endpoint without a user API key, and include the encrypted user credential only when one has been saved.
...
- Model update with `{ modelId, delete: true }` -> gateway removes only user-owned models and cleans primary/fallback assignments that referenced them. Gateway must reject deletion of admin-maintained directory models; if admin disables a directory model, normalization removes the non-custom row from user catalogs.
...
- Web helper tests cover the official Dicha provider branch and the user-owned provider/model branches used by settings UI to hide frontend sync/delete actions for admin-maintained catalog rows.
