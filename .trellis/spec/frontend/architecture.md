...
| 组件库 | **Lobe UI / Ant Design**（经 `DichaLobeProvider` 接入） | 前端基础组件体系 |
| 样式 | Tailwind v4 + dicha 语义 token + Lobe/AntD token bridge | CSS Modules 仅 bespoke 逃生口 |
...
## §5 样式 + token：Tailwind v4 + Lobe UI / Ant Design

- **Lobe UI / Ant Design** 是基础组件体系；通过 `DichaLobeProvider` 在 app 根部接入 Lobe `ConfigProvider` / `ThemeProvider`，并传入 `motion`。
- **语义 token 层**：`index.css` 用 `@theme inline` 把暖白哑光语义变量（`--surface`/`--ink`/`--hairline`/`--accent-*` 等）注册为 Tailwind 工具类；昼夜按 `data-theme` 换变量值。Lobe/AntD token 在 provider 与 CSS bridge 中映射到项目语义 token。详见 [design-system.md](./design-system.md)。
- **CSS Modules 降级为 bespoke 逃生口**（utility 难表达的像素效果，如 dithering / hard-edge）。
- 分层不变：**Lobe UI/AntD/React 管 DOM 的 HUD/表单/弹窗，PixiJS 管 canvas sprite**（M2）。两者共存。
...
- Lobe/AntD 输入是受控组件，用 RHF `<Controller>` 接。
...
- **Lobe UI / Ant Design 接入**：app 根部使用 `DichaLobeProvider`，内部包 `ConfigProvider motion={motion}` 与 `ThemeProvider`；关闭 Lobe 默认字体/global style，AntD token 通过 CSS 变量对齐项目主题。
...
- [x] Lobe/AntD theme ↔ 昼夜 palette 变量映射（见 `DichaLobeProvider` + `index.css` bridge）
- [ ] e2e 工具最终选型
