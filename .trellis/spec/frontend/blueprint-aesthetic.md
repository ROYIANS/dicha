...
.lp-outer-node-offset {
  --node-horizontal-offset: 12.5px;
} /* base: rail w-4=16px */
@media (min-width: 640px) {
  .lp-outer-node-offset {
    --node-horizontal-offset: 19.5px;
  }
} /* sm: w-6=24px */
@media (min-width: 768px) {
  .lp-outer-node-offset {
    --node-horizontal-offset: 43.5px;
  }
} /* md: w-12=48px */
@media (min-width: 1024px) {
  .lp-outer-node-offset {
    --node-horizontal-offset: 45.5px;
  }
} /* lg: 中心 48.5px = 1px 线中心 */
```
...
### 3.5.1 落地页局部纹样（PatternField）

> 落地页非 hero 区域可以使用更密的线性平铺纹样，但必须落在有结构意义的位置：演示窗口 chrome、读数条、台账列表、信笺侧栏、空白填充格等。

- 当前范本：`apps/web/src/routes/index.tsx` 的 `PatternField`，variant 为 `weave | ledger | ticks`。
- 颜色：全部 `stroke: currentColor`，外层 `color: var(--lp-deco)`；透明度通常 `0.09~0.22`，不得成为主视觉。
- 遮罩：每个实例必须有方向性 `mask-image` / `-webkit-mask-image`，从内容边缘渐隐；不要满屏均匀铺开。
- 层级：放在 `relative isolate overflow-hidden` 容器内，纹样 `absolute inset-0 z-0 pointer-events-none`，内容显式 `relative z-10`。
- 复用边界：只在落地页 1~2 处使用时可保留 route-local；如果设置页、登录页、dashboard 等也需要同款 variant，再抽到 `components/`，并继续用 `useId()` 避免 SVG pattern id 冲突。
- 禁止：用纹样替代布局结构、在相邻 section 重复同一 variant、添加 fake label / fake number / 坐标文字来解释纹样。
...
background: linear-gradient(
  to right,
  transparent,
  color-mix(in oklab, var(--lp-brand) 18%, transparent),
  transparent
);
```
...
.lp-btn-primary {
  ...
  box-shadow:
    inset 0 -2px 0 0 color-mix(in oklab, var(--sidebar-ink) 12%, transparent),
    /* 底边 inset 凸起 */ 0 1px 3px color-mix(in oklab, var(--sidebar-bg) 40%, transparent);
}
.lp-btn-primary:hover {
  background: color-mix(in oklab, var(--sidebar-bg) 88%, var(--sidebar-ink));
  box-shadow: none;
} /* 摊平 */
