...
- 为关键后台写操作接入审计：
  ...
  - 用户禁用/启用、强制退出会话。
- 完善超级管理员首页“大势报”：
  - 聚合平台规模、核心服务健康、宿主机/进程资源、维护待办和最近审计。
  - 首屏能快速判断 API、数据库、AI Gateway 是否平稳。
  - Dashboard 不展示菜单/模块清单；菜单归侧边栏负责，首页只展示关键数据、状态卡片和可判断系统趋势的信息。
- 完善系统运营工具：
  - 系统功能页按运维工具结构展示：系统状态、外部服务、数据备份、系统运行日志、缓存管理。
  - 系统状态包含 CPU、内存、磁盘、API 进程运行时长等摘要。
  - 外部服务包含 PostgreSQL、ClickHouse、Redis、本地存储、邮件服务、AI 服务等状态摘要。
  - 系统运行日志指 API / AI Gateway / 反向代理等运行输出，不等同于后台审计日志。
  - 支持从服务端配置的日志文件读取最近运行日志，并提供搜索。
  - 支持读取备份目录中的备份文件列表；配置备份命令后可从后台触发立即备份。
  - 支持安全维护动作：刷新健康检查、清理过期会话、查看审计日志、检查缓存状态、查看运行日志来源。
  - 重启 API / AI Gateway、清理运行时缓存等动作可以通过服务端环境变量配置命令后执行；未配置时不假装成功。
...
- [x] 首页“大势报”展示系统健康、资源占用、维护待办、外部服务和最近审计，不再展示菜单模块清单。
- [x] 系统功能页展示系统状态、外部服务、数据备份、系统运行日志、缓存管理和维护工具。
- [x] 系统功能页使用标签页工作台结构，而不是说明型卡片堆叠。
- [x] 运行日志支持从服务端配置的日志文件读取最近日志行。
- [x] 备份支持读取备份目录文件列表，并可通过配置命令触发立即备份。
- [x] 重启服务和清理缓存通过服务端显式配置命令启用；未配置时按钮禁用并说明原因。
